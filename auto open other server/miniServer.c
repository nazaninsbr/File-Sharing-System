...
void becomeAServer(File** head, ThisSystem** ourSystem){
  ...
  serverAddr.sin_port = htons((*ourSystem)->port);
  /* Set IP address to localhost */
  serverAddr.sin_addr.s_addr = inet_addr((*ourSystem)->ip);
  ...
  while(1){
  		...
  		if(listen(welcomeSocket,5)==0)
    		...
    		write(1, "Error with welcomeSocket\n", 25);
    		_exit(1);
  		}
        ...
        memset(outputbuff, 0, sizeof(outputbuff));
      	...
      	if(buffer[0]=='i'){
            decode_Resource_from_socket(head, buffer, sizeof(buffer));
        }
        //we got file request
        if(buffer[0]=='g'){

            handle_get_file(buffer, head, ourSystem, outputbuff);
            send(newSocket, outputbuff, sizeof(outputbuff), 0);
            memset(buffer, 0, sizeof(buffer));
        	memset(outputbuff, 0, sizeof(outputbuff));
        	buffer[0]='\0';
        	outputbuff[0]='\0';
